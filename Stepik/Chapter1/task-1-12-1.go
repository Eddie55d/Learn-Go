...
// Тип чисел, входящих в массив, должен соответствовать минимально
// возможному целому беззнаковому числу.
...
// (если такая пара чисел 3 и 7, значит в массиве элемент с 3 индексом нужно
// 	поменять местами с элементом, индекс которого 7).
...
func main() {
	...
	for i := 0; i < 3; i++ {
		...
		fmt.Scan(&index1)
		first := workArray[index1]

		fmt.Scan(&index2)
		second := workArray[index2]

		workArray[index1] = second
		workArray[index2] = first
	}

	for i := 0; i < 10; i++ {
		fmt.Printf("%v ", uint8(workArray[i]))
