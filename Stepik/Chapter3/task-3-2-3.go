...
// знаков после "запятой" (на самом деле после точки, результат не требуется
...
// Это вполне допустимый вариант, но если вы резонно обрабатываете ошибку
...
func main() {
	...
	strNoSpace := strings.ReplaceAll(str, " ", "")

	strNoComma := strings.ReplaceAll(strNoSpace, ",", ".")

	subStr := strings.Split(strNoComma, ";")

	subNum1 := subStr[0]
	subNum2 := subStr[1]

	num1, err := strconv.ParseFloat(subNum1, 64)
	if err != nil {
		println(err)
	}

	num2, err := strconv.ParseFloat(subNum2, 64)
	if err != nil {
		println(err)
	}
